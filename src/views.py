...
def get_time_interval(file_path: Path, user_date: str) -> DataFrame:
    """
...
def get_dict_card(file_path: Path, user_date: str) -> list:
    """
    Функция, которая собирает данные о картах,
    используемые в период времени указанный пользователем.
    :param file_path: путь к файлу с данными.
    ...
    for card in list_card:
        ...
        if type(card) == str:
            sum_card = round(
                abs(
                    operations.loc[
                        (operations["Номер карты"] == card) & (operations["Сумма операции"] <= 0), "Сумма операции"
                    ].sum()
                ),
                2,
            )
...
def get_top_transactions(file_path: Path, user_date: str) -> list:
    """
...
def get_currency_rate(file_json: Path) -> list | None:
    """
    ...
    try:
        ...
        user_settings = list()
    base_user = user_settings["user_currencies"]
    list_currency = list()
    for base in base_user:
        ...
        try:
            ...
            currency_rates["rate"] = response_json["rates"]["RUB"]
...
def get_stocks_rate(file_json: Path) -> list[dict[str, Any]]:
    """
    Функция, которая забирает данные через API и передает
    ...
    try:
        ...
        user_settings = list()
    stocks_user = user_settings["user_stocks"]
    list_stocks = list()
    for stocks in stocks_user:
        ...
        try:
            ...
            response_json = response.json()
            user_stocks["stock"] = response_json["data"][0]["close"]
            user_stocks["price"] = response_json["data"][0]["symbol"]
